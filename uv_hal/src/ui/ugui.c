...
{
   ug_fill_frame(0,0,gui->x_dim-1,gui->y_dim-1,c);
}
...
void ug_fill_round_frame( UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_S16 r, UG_COLOR c )
{
   ...
   ug_fill_frame(x1 + r, y1, x2 - r, y2, c);

   while ( x <= y )
   {
     if( y > 0 )
     {
        ug_draw_line(x2 + x - r, y1 - y + r, x2+ x - r, y + y2 - r, c);
        ug_draw_line(x1 - x + r, y1 - y + r, x1- x + r, y + y2 - r, c);
     }
     if( x > 0 )
     {
        ug_draw_line(x1 - y + r, y1 - x + r, x1 - y + r, x + y2 - r, c);
        ug_draw_line(x2 + y - r, y1 - x + r, x2 + y - r, x + y2 - r, c);
     }
     if ( xd < 0 )
     {
        xd += (x << 2) + 6;
...
{
   ug_draw_line(x1,y1,x2,y1,c);
   ug_draw_line(x1,y2,x2,y2,c);
   ug_draw_line(x1,y1,x1,y2,c);
   ug_draw_line(x2,y1,x2,y2,c);
}
...
void ug_draw_round_frame( UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_S16 r, UG_COLOR c )
{
   ...
   ug_draw_line(x1+r, y1, x2-r, y1, c);
   ug_draw_line(x1+r, y2, x2-r, y2, c);
   ug_draw_line(x1, y1+r, x1, y2-r, c);
   ug_draw_line(x2, y1+r, x2, y2-r, c);
   ug_draw_arc(x1+r, y1+r, r, 0x0C, c);
   ug_draw_arc(x2-r, y1+r, r, 0x03, c);
   ug_draw_arc(x1+r, y2-r, r, 0x30, c);
   ug_draw_arc(x2-r, y2-r, r, 0xC0, c);
}
...
void ug_fill_circle( UG_S16 x0, UG_S16 y0, UG_S16 r, UG_COLOR c )
{
   ...
   while ( x <= y )
   {
     if( y > 0 )
     {
        ug_draw_line(x0 - x, y0 - y,x0 - x, y0 + y, c);
        ug_draw_line(x0 + x, y0 - y,x0 + x, y0 + y, c);
     }
     if( x > 0 )
     {
        ug_draw_line(x0 - y, y0 - x,x0 - y, y0 + x, c);
        ug_draw_line(x0 + y, y0 - x,x0 + y, y0 + x, c);
     }
     if ( xd < 0 )
     {
...
void ug_put_string( UG_S16 x, UG_S16 y, char* str )
{
   ...
   while ( *str != 0 )
   {
      ...
      ug_put_char(chr, xp, yp, gui->fore_color, gui->back_color);
...
void ug_console_put_string( char* str )
{
   ...
   while ( *str != 0 )
   {
      ...
      if ( gui->console.y_pos+gui->font.char_height > gui->console.y_end )
      {
         ...
         ug_fill_frame(gui->console.x_start,gui->console.y_start,gui->console.x_end,gui->console.y_end,gui->console.back_color);
      }

      ug_put_char(chr, gui->console.x_pos, gui->console.y_pos, gui->console.fore_color, gui->console.back_color);
      str++;
   }
}
...
{
   // Frame 0
   ug_draw_line(xs, ys  , xe-1, ys  , *p++);
   ug_draw_line(xs, ys+1, xs  , ye-1, *p++);
   ug_draw_line(xs, ye  , xe  , ye  , *p++);
   ug_draw_line(xe, ys  , xe  , ye-1, *p++);
   // Frame 1
   ug_draw_line(xs+1, ys+1, xe-2, ys+1, *p++);
   ug_draw_line(xs+1, ys+2, xs+1, ye-2, *p++);
   ug_draw_line(xs+1, ye-1, xe-1, ye-1, *p++);
   ug_draw_line(xe-1, ys+1, xe-1, ye-2, *p++);
   // Frame 2
   ug_draw_line(xs+2, ys+2, xe-3, ys+2, *p++);
   ug_draw_line(xs+2, ys+3, xs+2, ye-3, *p++);
   ug_draw_line(xs+2, ye-2, xe-2, ye-2, *p++);
   ug_draw_line(xe-2, ys+2, xe-2, ye-3, *p);
}
...
UG_RESULT ug_window_create( UG_WINDOW* wnd, UG_OBJECT* objlst, UG_U8 objcnt, void (*cb)( UG_MESSAGE* ) )
{
   ...
   wnd->xs = 0;
   wnd->ys = 0;
   wnd->xe = ug_get_x_dim()-1;
   wnd->ye = ug_get_y_dim()-1;
   wnd->cb = cb;
...
UG_RESULT ug_window_resize( UG_WINDOW* wnd, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye )
{
   ...
   if ( (wnd != NULL) && (wnd->state & WND_STATE_VALID) )
   {
      ...
      {
         if ( wnd->ys ) ug_fill_frame(0, 0, xmax,wnd->ys-1,gui->desktop_color);
         pos = wnd->ye+1;
         if ( !(pos > ymax) ) ug_fill_frame(0, pos, xmax,ymax,gui->desktop_color);
         if ( wnd->xs ) ug_fill_frame(0, wnd->ys, wnd->xs-1,wnd->ye,gui->desktop_color);
         pos = wnd->xe+1;
         if ( !(pos > xmax) ) ug_fill_frame(pos, wnd->ys,xmax,wnd->ye,gui->desktop_color);
...
{
   UG_COLOR c;
   c = ug_window_get_title_text_color( wnd );
   if ( ug_window_set_title_text_color( wnd, ug_window_get_title_color( wnd ) ) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
   if ( ug_window_set_title_color( wnd, c ) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
   return UG_RESULT_OK;
}
...
UG_RESULT ug_window_set_x_start( UG_WINDOW* wnd, UG_S16 xs )
{
   ...
   {
      wnd->xs = xs;
      if ( ug_window_resize( wnd, wnd->xs, wnd->ys, wnd->xe, wnd->ye) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
      return UG_RESULT_OK;
   }
   return UG_RESULT_FAIL;
}
...
UG_S8 ug_window_get_title_text_h_space( UG_WINDOW* wnd )
{
   UG_S8 hs = 0;
   if ( (wnd != NULL) && (wnd->state & WND_STATE_VALID) )
   {
...
UG_RESULT _UG_WindowDrawTitle( UG_WINDOW* wnd )
{
   ...
   if ( (wnd != NULL) && (wnd->state & WND_STATE_VALID) )
   {
      ...
      /* Draw title */
      ug_fill_frame(xs,ys,xe,ys+wnd->title.height-1,txt.bc);
      ...
      /* Draw line */
      ug_draw_line(xs,ys+wnd->title.height,xe,ys+wnd->title.height,pal_window[11]);
      return UG_RESULT_OK;
   }
   return UG_RESULT_FAIL;
}
...
void _UG_WindowUpdate( UG_WINDOW* wnd )
{
   ...
   if ( wnd->state & WND_STATE_VISIBLE )
   {
      ...
      /* Draw window area? */
      ug_fill_frame(xs,ys,xe,ye,wnd->bc);
   ...
   {
      ug_fill_frame(wnd->xs,wnd->xs,wnd->xe,wnd->ye,gui->desktop_color);
   }
}
...
UG_RESULT _UG_WindowClear( UG_WINDOW* wnd )
{
   if ( wnd != NULL )
   {
      if (wnd->state & WND_STATE_VISIBLE)
      {
         ...
         ug_fill_frame( wnd->xs, wnd->ys, wnd->xe, wnd->ye, gui->desktop_color );
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( (obj->state & OBJ_STATE_REDRAW) || (btn->state & BTN_STATE_ALWAYS_REDRAW) )
         {
            ...
            ug_fill_frame(obj->a_abs.xs+d, obj->a_abs.ys+d, obj->a_abs.xe-d, obj->a_abs.ye-d, txt.bc);
         ...
         {  /* 2D */
            ug_draw_frame(obj->a_abs.xs,obj->a_abs.ys,obj->a_abs.xe,obj->a_abs.ye,(btn->state&BTN_STATE_PRESSED)?btn->abc:btn->afc);
         }
      }
      else
      {
         ug_fill_frame(obj->a_abs.xs, obj->a_abs.ys, obj->a_abs.xe, obj->a_abs.ye, wnd->bc);
      }
      obj->state &= ~OBJ_STATE_UPDATE;
   }
}
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( obj->state & OBJ_STATE_REDRAW )
         {
            ...
            ug_fill_frame(obj->a_abs.xs, obj->a_abs.ys, obj->a_abs.xe, obj->a_abs.ye, txt.bc);
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( obj->state & OBJ_STATE_REDRAW )
         {
            ...
            {
               ug_draw_bmp(obj->a_abs.xs,obj->a_abs.ys,(UG_BMP*)img->img);
            }
