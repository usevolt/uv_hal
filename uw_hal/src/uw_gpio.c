...
static void set_iocon(uw_gpios_e gpio, unsigned int value);
...
void PIOINT1_IRQHandler (void) {
	...
	for (i = 0; i < 12; i++) {
		...
		if ((LPC_GPIO1->MIS & (1 << i)) >> i) {
			if (callback) {
				callback(__uw_get_user_ptr(), (uw_gpios_e) (GPIO_PORT_1 + i));
				// clear interrupt on this pin only
				LPC_GPIO1->IC |= (1 << i);
			}
		}
...
void PIOINT3_IRQHandler (void) {
	...
	for (i = 0; i < 4; i++) {
		...
		if ((LPC_GPIO3->MIS & (1 << i)) >> i) {
			if (callback) {
				callback(__uw_get_user_ptr(), (uw_gpios_e) (GPIO_PORT_3 + i));
				// clear interrupt on this pin only
				LPC_GPIO3->IC |= (1 << i);
			}
		}
