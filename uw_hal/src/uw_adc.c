...
void uw_init_adc(unsigned int channels, int fosc, uw_adc_modes_e mode) {
	...
	case ADC_STANDARD_MODE:
		...
		break;
	default:
		printf("Error: Unknown ADC mode %u\n\r", mode);
		return;
	}
...
int uw_read_adc(unsigned int channel) {
...
int uw_read_adc_average(unsigned int channel, unsigned int conversion_count) {
	int value = 0, i;
